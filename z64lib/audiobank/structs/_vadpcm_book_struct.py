...
class VadpcmBook(DynaStruct):
    ...
    def __init__(self, header=None, predictors=None):
        self.header = header or VadpcmBookHeader()
        self.predictors = predictors or array[s16]([])

    @classmethod
    def from_bytes(cls, buffer: bytes, offset: int = 0) -> 'VadpcmBook':
        header = VadpcmBookHeader.from_bytes(buffer, offset)

        order = header.order
        num_predictors = header.num_predictors
        total_coeff = 8 * order * num_predictors

        p_offset = offset + header.size()
        predictors = array[s16].from_bytes(buffer, p_offset, total_coeff)
